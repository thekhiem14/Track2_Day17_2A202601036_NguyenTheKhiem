...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    ON CONFLICT (event_id) DO UPDATE: nếu message bị phát lại (do restart sau
    crash), ghi đè bằng nội dung mới thay vì tạo thêm hàng -> idempotent.
    Cần PRIMARY KEY trên event_id (khai ở DDL) thì ON CONFLICT mới hợp lệ.
    """
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # at-least-once: ghi dữ liệu TRƯỚC, commit offset SAU. Nếu crash
            # xảy ra ở maybe_crash() (giữa hai bước), dữ liệu đã ghi nhưng
            # offset CHƯA dịch -> lần khởi động lại sẽ đọc lại đúng lô này.
            # An toàn vì write_batch() giờ là idempotent (ON CONFLICT DO
            # UPDATE) — phát lại không tạo thêm hàng.
